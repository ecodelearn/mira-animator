...
- **3D, QR, quizzes & images:** drop a real, auto-rotating 3D element with `/mira-3d`, a scannable QR code (from a link or text) with `/mira-qrcode`, a live quiz with presenter-controlled correct-answer reveal using `/mira-quiz`, a live post-it wall of free-text answers with `/mira-postit`, or an image you already have with `/mira-image`. A 3D slide that loads a `.glb` needs a local server (the agent starts one and writes a double-click launcher); everything else opens from `file://`.
